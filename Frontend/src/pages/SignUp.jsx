...
function SignUp() {
  ...
  const googleSignUp = async()=>{
    try{
      ...
      dispatch(setUserData(result.data))
      navigate("/")
      toast.success("Signup Successfully via Google")
     ...
     h-[100vh] flex items-center justify-center'>
        <form className='w-[90%] md:w-200 h-150 bg-[white] shadow-xl rounded-2xl flex' onSubmit={(e)=>e.preventDefault()}>
          ...
          <div className='md:w-[50%] w-[100%] h-[100%] flex flex-col items-center justify-center gap-3'>
            <div>
              <h1 className='font-cemibold text-[black] text-2xl'>Let's get started</h1>
            ...
            <div className='flex md:w-[50%] w-[70%] items-center justify-between'>
              <span className={`px-[10px] py-[50] border-[2px] border-[#e7e6e6] rounded-2xl cursor-pointer hover:border-black ${role === "student" ? "border-black" : "border-[#646464"}`} onClick={()=>setRole("student")}>Student</span>
              <span className={`px-[10px] py-[50] border-[2px] border-[#e7e6e6] rounded-2xl cursor-pointer hover:border-black ${role === "educator" ? "border-black" : "border-[#646464"}`} onClick={()=>setRole("educator")}>Educator</span>
            </div>
