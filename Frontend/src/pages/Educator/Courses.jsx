          ...
          <tbody >
            ...
            <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
              <td className='py-3 px-4 flex items-center gap-4'>
                {course?.thumbnail ? <img src={course?.thumbnail} className='w-24 h-14 object-cover rounded-md' alt="" /> : 
                <img src={img} className='w-24 h-14 object-cover rounded-md' alt="" />}<span>{course?.title}</span>
              </td>
              ...
              <td className='px-4 py-3'><span className={`px-3 py-1 rounded-full text-xs 
                ${course.isPublished ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>{course?.isPublished ? "Published" : "Draft"}</span></td>
