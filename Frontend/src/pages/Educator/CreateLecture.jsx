    ...
    const handleCreateLecture = async () => {
        ...
        try {
            const result = await axios.post(
                serverUrl + `/api/course/createlecture/${courseId}`,   // ✅ FIXED
                { lectureTitle },
    ...
    return (
        <div className='min-h-screen bg-gray-100 flex items-center justify-center p-4'>
            <div className='bg-white shadow-xl rounded-xl w-full max-w-2xl p-6'>
                ...
                {/* buttons */}
                <div className='flex gap-4 mb-6'>   {/* ✅ FIXED */}
                    <button
                        className='flex items-center gap-2 px-4 py-2 rounded-md bg-gray-200 hover:bg-gray-300 
                        text-sm font-medium'
                        onClick={() => navigate(`/editcourse/${courseId}`)}
                    >
                        <FaEdit /> Back to Course
                    </button>

                    <button
                        className='px-5 py-2 rounded-md bg-[black] text-white hover:bg-gray-600 transition-all 
                ...
                <div className='space-y-2'>
                    {lectureData?.map((lecture, index) => (
                        ...
                        >
                            ...
                            <FaEdit
                                className='text-gray-500 hover:text-grey-700 cursor-pointer'
